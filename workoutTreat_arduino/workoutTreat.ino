...
void loop() {
  ...
  if (BPM <= 60) {
    lcd.clear();
    lcd.print("ARE YOU A ROBOT?");
    myservo.write(0);
  }

  if (BPM > 60 && BPM <= 100) {
    lcd.clear();
    lcd.print("GO WORKOUT FIRST");
    lcd.setCursor(0, 1);
    lcd.print("THEN WE'll SEE!!");
    myservo.write(0);
  }

  if (BPM > 100 && BPM <= 140 && (BPM - preBPM) > 0) {
    lcd.clear();
    lcd.print("GOOD JOB!!! ");
    lcd.setCursor(0, 1);
    lcd.print("YOU MAY OPEN IT");
    myservo.write(90);
    delay(3000);
  }
  /*
    if (BPM <= 200 && BPM > 140) {
      lcd.clear();
      lcd.print("ARE YOU A SUPERROBOT?");
      myservo.write(0);
    }
    */
  preBPM = BPM;
